...
class InvoiceService
{
    ...
    public function getAllInvoicesData($request)
    {
        $invoice = Invoice::query()
            ...
            ->leftJoin('gateways', 'orders.gateway_id', '=', 'gateways.id')
            ->leftJoin('file_managers', ['orders.deposit_slip_id' => 'file_managers.id', 'file_managers.origin_type' => (DB::raw("'App\\\Models\\\Order'"))])
            ->select(['invoices.*','orders.payment_method','orders.slip_document', 'gateways.title as gatewayTitle', 'gateways.slug as gatewaySlug', 'file_managers.file_name', 'file_managers.folder_name']);
            ...
            if ($request->status == INVOICE_STATUS_PAID) {
                ...
                if (is_null($order)) {
                    ...
                    $order = Order::create([
                        ...
                        'deposit_slip_id' => $deposit_slip_id?$deposit_slip_id:null,
                        'gateway_id'=>$request->payment_method=='bank'?10:5,
