...
var dt = $('#allDatatable').DataTable({
    ...
    footerCallback: function (row, data, start, end, display) {
        ...
        $(api.column(4).footer()).html('All Total : ' + currencyPrice(api.ajax.json().total) + ' (Tax : ' + currencyPrice(api.ajax.json().tax_amount) + ')');
        $(api.column(5).footer()).html('Page Total : ' + currencyPrice(totalTaxAmount));
        $(api.column(6).footer()).html('Page Total : ' + currencyPrice(totalAmount));
    },
    columns: [{
        "data": 'DT_RowIndex',
        "name": 'DT_RowIndex',
